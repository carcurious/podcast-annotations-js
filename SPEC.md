...
- **Apple Podcasts Timed Links** (iOS 26.2, Nov 2025). Apple's own timestamped link banners — rendered on the Now Playing screen, inline in the transcript, and in a "From This Episode" section — are the largest-scale validation yet that specific instants in an episode deserve addressable UI, not just the file as a whole. The feature is materially narrower than this spec (link destinations limited to Apple's own ecosystem and other podcasts; no typed entities, canonical IDs, or layers). See [Relationship to Other Standards](#relationship-to-other-standards).
...
**Apple Podcasts Timed Links.** Starting with iOS 26.2 (Nov 2025), Apple Podcasts renders creator-authored timed links as banners on the Now Playing screen, inline in the transcript, and in a "From This Episode" section on the episode page. There are two authoring paths: hyperlinked text and a timestamp in the episode description, or `url` fields on the existing Podcasting 2.0 `<podcast:chapters>` JSON — Apple introduced no new RSS tag for this. Apple also auto-detects when a host mentions another podcast (English-language shows, opt-out via Apple Podcasts Connect) and links to that show automatically.

The feature validates the moment-level pattern this spec is built on, but at much narrower scope: link destinations are limited to Apple's own ecosystem (Apple Books, Music, Music Classical, Maps, News, Podcasts, Sports, Stocks, TV, Shazam) plus other podcasts — never an arbitrary URL. There is no typed entity (`person`, `place`, `car`, …), no `canonicalId`, no `layer`/`producer`, and no time *span*: a timed link fires at a point, not across the range an entity is actually discussed. Apple has documented the rendering behavior but not published a data model for what a timed link is.

*Interop guidance:* because Apple already renders `url` fields on `<podcast:chapters>`, a producer can export a slice of their highest-`priority` annotations into a chapters file and get Apple Podcasts to render them as timed links today, with no new infrastructure. Treat this as a lossy, one-way export for the editorial highlights, not a transport for the full set — a chapters entry carries no `type`, `canonicalId`, `speaker`, or `confidence`, chapters are coarse and non-overlapping by design, and iOS 26.2 only auto-generates chapters when a feed provides none, so publishing every annotation as a chapter would suppress that. The full annotation set stays in the sidecar file or, per the RSS Distribution guidance below, a `<podcast:annotations>` element.

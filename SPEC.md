...
A `canonicalId` is a join key: once assigned, it SHOULD NOT change, because renaming an identifier silently breaks every earlier file that used it. Because the producer mints the identifier, one always exists, at whatever granularity the producer needs, with no dependency on any external registry.

#### Wikidata QIDs

Producers MAY use external identifiers such as [Wikidata](https://www.wikidata.org/wiki/Wikidata:Identifiers) QIDs as `canonicalId` values (e.g., `wikidata:Q354043` for Carroll Shelby, `wikidata:Q152207` for the Nürburgring). The namespaced convention above is scoped to the producer's own corpus; a QID adds a language-independent identity that uncoordinated producers can arrive at independently, so it is the stronger choice when annotations should join across shows and producers.

Coverage is strong for well-known entities, including at the generation level enthusiast conversation sometimes needs: this spec's own automotive example set resolves to items for its cars, people, and places (Honda Prelude `Q830370`, Toyota Supra `Q647404`, Carroll Shelby `Q354043`), and where an episode distinguishes a specific generation, items like BMW M5 (F90) `Q56316510` or Toyota Supra (A80) `Q2447212` exist alongside the nameplate items. Coverage is not total: the same test found no item for FCP Euro, a parts retailer and podcast sponsor, and no entity matching "solenoid handles", a colloquial name for electronic door-handle hardware. Podcast conversation is dense with exactly this mid-tail, so producers adopting QIDs still need the namespaced form as a fallback (`company:fcp-euro`), and per the stability rule above, an entity keyed with a namespaced identifier keeps it even if Wikidata later gains a matching item.

To find a QID in practice, search the entity name on [wikidata.org](https://www.wikidata.org) or call the [`wbsearchentities` API](https://www.wikidata.org/w/api.php?action=help&modules=wbsearchentities); bulk pipelines can use the [SPARQL query service](https://query.wikidata.org) or a reconciliation service. Search matches label and alias prefixes, so an entity can exist under a label the producer did not try (the GM LS engine, `Q724143`, does not surface for the query "LS engine"), and a name can surface unrelated items first ("solenoid" returns a novel before the electromechanical device). Producers SHOULD verify an item's description before adopting it as an identifier.

*Non-normative:* the interest in shared identifiers comes from outside the project. John Spurlock (OP3): "it would be great if podcast publishers delineated mentioned entities using stable identifiers (useful across shows/episodes) with something agreed upon like wikidata ids." Nathan Gathright, arguing that most new podcast tags lose to client-side inference from transcripts, drew an exception that covers this field: "Tags that express publisher intent, payments, identity, attribution, or canonical relationships still provide something apps shouldn't just guess." A canonical identifier is publisher-stated identity, which a consumer cannot reliably infer from audio. A dedicated field for external identity assertions (Schema.org `sameAs` semantics) is under consideration for a future version.

...
- `SPEC.md`: new [Publisher Signals (Open Question)](SPEC.md#publisher-signals-open-question) section after Layers. Records that independent layers require no coordination with the publisher of the audio, that the two closest shipping features (YouTube's per-video automatic-concepts checkbox, Apple's Podcasts Connect opt-out for auto-detected mentions) both answer this with a platform-internal per-item control, and that an open format has no equivalent chokepoint, so any signal would be advisory. No mechanism is defined; the section explains why it belongs to the Podcasting 2.0 namespace process. Non-normative, no data-model change, spec version remains `1.2.0`.

...
### Added
- Non-normative **Wikidata QID guidance** in `SPEC.md`'s Canonical IDs section: when a QID makes the stronger `canonicalId` (cross-producer joins without coordination), practical lookup guidance (`wbsearchentities`, SPARQL, prefix-search pitfalls), live-tested coverage against the Everyday Driver example set (10 of 12 entities resolve; FCP Euro and "solenoid handles" do not, so the namespaced form stays the fallback), and an explicit stability rule: a `canonicalId` never changes form once assigned. Motivated by John Spurlock's ask for publisher-delineated entities with agreed-upon stable identifiers such as Wikidata IDs. No data-model change; spec version remains `1.1.0`.
